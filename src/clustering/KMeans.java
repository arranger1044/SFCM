...
//    int[] mask = stack.getProcessor(1).getMask();
    ...
    /// PRENDE IL CLUSTER + VICINO AL CLUSTER X.  X = INSIEME DI CARATTERISTICHE O DESCRITTORI.
...
//        Validate.isTrue(x.length == vp.getNumberOfValues(),   // CONTROLLA CHE IL NUMERO DELLE CARATTERISTICHE SIA UGUALE.
...
//    }
    ...
    private ByteProcessor encodeSegmentedImage() {
        ...
        while (iterator.hasNext()) {
            ...
            final int c = KMeansUtils.closestCluster(v, clusterCenters);  // PRENDE IL CLUSTER + VICINO AL PIXEL
            dest.putPixel(iterator.getX(), iterator.getY(), c); // SCRIVE IL PIXEL NELLA DESTINAZIONE.
        }
        return dest;
    }
    ...
    private ImageStack encodeCentroidValueImage() {
        return KMeansUtils.encodeCentroidValueImage(clusterCenters, vp);  /// CODIFICA IL CENTROIDE AGGIUNGENDO VP .
    }
    ...
    private void cluster() {
        ...
        while (!converged) {
            ...
            final MeanElement[] newClusterMeans = new MeanElement[config.getNumberOfClusters()]; //CREA UN VETTORE DI CENTROIDI
            ...
            while (iterator.hasNext()) {
                final float[] v = iterator.next();
                final int c = KMeansUtils.closestCluster(v, clusterCenters); /// PRENDE IL CLUSTER + VICINO A  V   E  AGGIUNGE A QUESTO CLUSTER V
            ...
            for (int i = 0; i < clusterCenters.length; i++) {
                ...
                final float[] newClusterCenter = newClusterMeans[i].mean();  // AGGIORNA LA MEDIA DEI NUOVI CLUSTER
                distanceSum += KMeansUtils.distance(clusterCenter, newClusterCenter); // X VEDERE SE LA DISTANZA è MIGLIORATA
            }

            converged = distanceSum < config.getTolerance();

            for (int i = 0; i < clusterCenters.length; i++) {
                clusterCenters[i] = newClusterMeans[i].mean(); // AGGIORNA LA MEDIA E L'ASSENGA AI CLUSTER.
            }
    ...
    private float[][] initializeClusterCenters() {
        ...
        final List<Point> centerLocation = new ArrayList<Point>(); // contiene i centri già calcolati
        ...
        while (centers.size() < nbClusters) {
            ...
            for (int offset = 0; offset < nbPixels; offset++) {  // per ogni pixel calcola quello più vicino, fai la distanza fai la potenza della distanza
                                                                 // aggiungi alla distribuzione di probabilità del pixel la somma.
                final Point p = toPoint(offset, width);

                // Test that this is not a repeat of already selected center // non ricalcola la distanza con i vecchi centri.
